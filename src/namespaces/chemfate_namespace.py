...
from utils.model_setup import Model_SetUp
...
scenario_model = simulationsNamespace.model('Simulation', {
    ...
    'materialID': fields.String(description="ID of the nanomaterial"),
    'regionID': fields.String(description="ID of the region"),  
})
...
class MainClass(Resource):
    ...
    def post(self):
        ...
        material = self.enm_dao.find_one({'userId': userid, '_id': simulation['materialID']})
        region = self.region_dao.find_one({'userId': userid, '_id': simulation['regionID']})
        
        model = Model_SetUp(simulation['date'], pbpkDays, 1, 10, material, region, simulation['emissions'])
        ...
        p = Process(target=model.run_model(), args=(simulation, task, userid))
        p.start()
...
#         token = token.split(" ")[1]
#         userid = oidc.user_getfield('sub', token)
#         simulation: input_Ion = request.json
#         simulation['userId'] = userid
#         simulation['chem_type'] = 'NonionizableOrganic'
#         simId = randomString(14)
#         simulation['_id'] = simId
...
#         task['userId'] = userid
#         task['messages'] = ['Starting simulation']
#         task['simulationKeys'] = ['NonionizableOrganic']
#         task['percentage'] = 0.05
#         mongoClient['task'].insert_one(task)
#         # _emission = mongoClient['prediction'].insert_one(prediction).inserted_id
#         mongoClient['chemFate'].insert_one(simulation)

#         with open('/src/data/chemFate_data/region.json') as f:
#             region = ast.literal_eval(f.read())

#         model = Model_SetUp(simulation['start_date'],simulation['end_date'],simulation['run_option'],
#                             simulation['bgPercOption'],simulation['chem_type'], simulation['params'], 
#                             region, simulation['release'])
