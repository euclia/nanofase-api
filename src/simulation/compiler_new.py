...
class Compiler:
        ...
        if extra_dims is not None:
            for dim in extra_dims:
                if dim[0] in dims:
                    ...
                    print("Cannot find extra dimension {0} for NetCDF variable {1} in config file.".format(dim[0],
                                                                                                           var_name))
        ...
        if var_name == 'land_use':
            ...
            if var_dict['type'] == 'raster':
                ...
                if '{' + record_dim + '}' in var_dict['path']:
                    print(
                        "Sorry, record dimension of spatial 1d variables must be given as separate bands, for the moment.")
                else:
                    print("One band per record dim")
            else:
                print(
                    "Unrecognised file type {0} for variable {1}. Type should be raster for 1d spatial variables.".format(
                        config[var]['type'], var))
    ...
    def parse_spatiotemporal_var(self, var_name):
        ...
        if var_dict['type'] == 'raster':
            ...
            if '{t}' in var_dict['path']:
                ...
                print(
                    "Spatiotemporal variable ({0}) in raster format must be provided by one raster file per time step, with path denoted by /{t/}".format(
                        var_name))
            ...
            for t in range(1, df['t'].max() + 1):
                ...
                if pt.shape != self.grid.shape:
                    print(
                        "Inconsistent shape between {0} csv file and overall grid system ({1} and {2}). Check indices set correctly.".format(
                            var, pt.shape, grid.shape))
                    sys.exit()
            ...
            print("Unrecognised file type {0} for variable {1}. Type should be raster or csv.".format(var_dict['type'],
                                                                                                      var_name))
    ...
    def parse_shapefile(self, var_name, units):
        ...
        if 'temporal_profile' in var_dict:
            ...
            df = df[(df['ISO3'] == self.config['iso3'].upper()) & (
            df[var_dict['temporal_profile']['source_type_col']] == var_dict['temporal_profile']['for_source_type'])]
            temporal_factors_data = df[var_dict['temporal_profile']['factor_col']].tolist()
            # Do the interpolation
            temporal_factors = np.interp(
                np.arange(0, int(self.config['time']['n']) * int(self.config['time']['dt']),
                          int(self.config['time']['dt'])),  # The desired x temporal res
        ...
        values = np.ma.array(np.ma.empty((10, int(self.config['time']['n']), *self.grid.shape), dtype=np.float64),
                             mask=True)
        ...
        for index, point in gdf.iterrows():
            if self.in_model_domain(point['geometry']):
                ...
                i = int(
                    ((int(point['geometry'].x) - int(point['geometry'].x) % self.grid.res[0]) - self.grid.bounds.left) /
                    self.grid.res[0])
                j = int(
                    (self.grid.bounds.top - (int(point['geometry'].y) - int(point['geometry'].y) % self.grid.res[1])) /
                    self.grid.res[1]) - 1
                ...
                while values[p, 0, j, i] is not np.ma.masked:
                    ...
                    if p >= values.shape[1]:
                        print(
                            "Maximum of {0} point sources allowed per cell, but cell {1},{2} (x,y zero-indexed) has more than that.".format(
                                values.shape[1], i, j))
                        sys.exit()
                # Which temporal profile should be applied?
                if ('temporal_profile' in var_dict) and \
                                point['temporalPr'] == var_dict['temporal_profile']['for_source_type']:
    ...
    def routing(self):
        ...
        inflows_arr = np.ma.array(np.ma.empty((*self.flow_dir.shape, 7, 2), dtype=np.dtype('i2')),
                                  mask=True)  # Max of seven inflows
