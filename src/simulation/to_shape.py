...
class GeoJ:
    ...
    def __parseGeoJ(self):
        ...
        self.geometryType = self.geoJObj['geometry']['type']

        self.columnsList = self.geoJObj['properties'].keys()

        # for i in self.geoJObj['features']:

        if self.geoJObj['geometry']['type'] == self.geometryType:
            self.geometries.append(self.geoJObj['geometry']['coordinates'])
            for j in self.columnsList:
                self.__attributesPerF.append(str(self.geoJObj['properties'][str(j)]))
...
#     gJ = GeoJ('input/polys.geojson')
#
#     # Creating a shapefile from the geoJSON object
#     gJ.toShp('output/polys')