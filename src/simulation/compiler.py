        ...
        if var_name == 'land_use':
            ...
            if var_dict['type'] == 'raster':
                ...
                if '{' + record_dim + '}' in var_dict['path']:
                    print("Sorry, record dimension of spatial 1d "
                          "variables must be given as separate bands, for the moment.")
                else:
                    print("One band per record dim")
            else:
                print(
                    "Unrecognised file type for variable . Type should be raster for 1d spatial variables.")
    ...
    def parse_spatiotemporal_var(self, var_name):
        ...
        if var_dict['type'] == 'raster':
            ...
            if '{t}' in var_dict['path']:
                ...
                print(
                    "Spatiotemporal variable ({0}) in raster format must be provided"
                    " by one raster file per time step, with path denoted by /{t/}".format(
                        var_name))
            ...
            for t in range(1, df['t'].max() + 1):
                ...
                if pt.shape != self.grid.shape:
                    print("Inconsistent shape between csv file and overall grid system. Check indices set correctly.")
                    sys.exit()
                # Should the array be clipped?
                if 'clip' in var_dict:
                    try:
    ...
    def parse_shapefile(self, var_name, units):
        ...
        for index, point in gdf.iterrows():
            if self.in_model_domain(point['geometry']):
                ...
                if ('temporal_profile' in var_dict) and point['temporalPr'] == var_dict['temporal_profile']['for_source_type']:
