...
from utils.load_data_nano import load_data
from utils.model_solver import org_solver, nano_solver #ion_solver
from utils.generate_result import GenerateResult


class Model_SetUp:

    def __init__(self, start_date: str, days: int, run_option: int, bgPercOption2: int,
                 material: dict, region: dict, release):
    ...
    def bgConc_new_cal(self, input_file_list, HL_air, HL_fWater, HL_fSedS):
        ...
        if self.material['type'] != 'NonionizableOrganic':
            bgConc_new_i = {}
    ...
    def run_model(self):
        ...
        if self.material['type'] != 'Nanomaterial':
            # load data
            data = LoadData(self.material, self.region, self.release, self.start_date, self.days)
            ...
            # load data
            time, presence, env, climate, bgConc, chemParams, release, release_scenario = load_data(self.region, self.release, self.material, self.start_date, self.days)
        ...
        if self.run_option == 1:
            ...
            # elif self.material['type'] == 'IonizableOrganic' or self.material['type']=='Metal':
            ...
            #         ion_solver(self.material['type'], self.start_date, sim_days, presence, env, climate, chemParams, bgConc, release)
            ...
            elif self.material['type'] == 'Nanomaterial':
        ...
        print(result.store_output(self.material['type'], chemParams['name'], env['name'], release_scenario, release,
