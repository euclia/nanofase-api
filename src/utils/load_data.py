...
class LoadData:
    ...
    def __init__(self, params, region, release, start_date, days):
        self.chem_type = params['type']
        self.params = params
    ...
    def get_Koc_acid(self, smiles, cas):
        ...
        Koc_acid = None
        df = pd.read_excel('../data/chemFate_data/IonizableChem_DB.xlsx', sheet_name='Koc_organicAcid')
    ...
    def get_infil_rate(self, soil_type, slope):
        infil_rate = None
        slope_col = None
        df = pd.read_excel('../data/chemFate_data/IonizableChem_DB.xlsx', sheetname='infiltrationRate', index_col=0)
    ...
    def load_compart_presence(self):
        ...
        return OrderedDict(self.region['Presence'])


    def load_env_params(self):
        # load the environmental parameters
        united = {}
        for key in list(self.region['Environment'].keys())[1:]:
            united.update(self.region['Environment'][key])
            
        env = OrderedDict()
        env['name'] = self.region['Environment']['name']
    ...
    def load_climate(self):
        ...
        climate_ws = self.region['Climate']
        climate_month = climate_ws['month']
        climate_day = climate_ws['day']
        climate_year = climate_ws['year']
        # precipitation unit: mm/day
        climate_precip = climate_ws['precipitation']
        # windspeed unit: m/second
        climate_windspeed = climate_ws['windspeeed']
        # water flow unit: m3/second
        climate_flow = climate_ws['waterflow']
        # temperature unit: C
        climate_temp = climate_ws['temperature']
        # evaporation unit: mm
        climate_evap = climate_ws['evaporation']

        date = zip(climate_year, climate_month, climate_day)
    ...
    def load_bg_conc(self, chem_params):
        ...
        bgConc = self.release['bgConc']

        for key, value in bgConc.items():
            # unit conversion from kg/m^3 to mol/m^3
            # kg/m3 / kg/mol = mol/m3
            bgConc[key] = value/chem_params['molar_mass']
    ...
    def load_release(self, chem_params, presence):
        ...
        dt = zip(self.release['year'], self.release['month'], self.release['day'])

        release = {}
        release['dates'] = dt

        # mol/day
        release['air'] = [(x / chem_params['molar_mass'])  for x in self.release['air']]
        release['fw'] = [(x / chem_params['molar_mass']) for x in self.release['freshwater']]
        release['fSS'] = [(x / chem_params['molar_mass']) for x in self.release['freshwater_ss']]
        release['fwSed'] = [(x / chem_params['molar_mass']) for x in self.release['freshwater_sed']]
        release['sw'] = [(x / chem_params['molar_mass'])  for x in self.release['seawater']]
        release['sSS'] = [(x / chem_params['molar_mass']) for x in self.release['seawater_ss']]
        release['swSed'] = [(x / chem_params['molar_mass']) for x in self.release['seawater_sed']]
        release['soil1'] = [(x / chem_params['molar_mass']) for x in self.release['undeveloped_soil']]
        release['dsoil1'] = [(x / chem_params['molar_mass']) for x in self.release['undeveloped_deep_soil']]
        release['soil2'] = [(x / chem_params['molar_mass']) for x in self.release['urban_soil']]
        release['dsoil2'] = [(x / chem_params['molar_mass']) for x in self.release['urban_deep_soil']]
        release['soil3'] = [(x / chem_params['molar_mass'])  for x in self.release['agricultural_soil']]
        release['dsoil3'] = [(x / chem_params['molar_mass']) for x in self.release['agricultural_deep_soil']]
        release['soil4'] = [(x / chem_params['molar_mass']) for x in self.release['agricultural_soil_biosolid']]
        release['dsoil4'] = [(x / chem_params['molar_mass']) for x in self.release['agricultural_deep_soil_biosolid']]

        return release, self.release['scenario']
